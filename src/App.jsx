...
:root {
  --bg: #0d0f0e; --surface: #151817; --surface2: #1c201e; --surface3: #232826;
  --border: rgba(255,255,255,0.07); --border2: rgba(255,255,255,0.12);
  --accent: #b8f06a; --accent2: #7ee8a2; --accent-dim: rgba(184,240,106,0.12);
  --text: #f0f2ee; --text2: #9aa396; --text3: #627065;
  --danger: #ff6b6b; --danger-dim: rgba(255,107,107,0.12);
  --warning: #ffc46b; --info: #6bbfff; --info-dim: rgba(107,191,255,0.12);
...
input, textarea, select {
  ...
  padding: 9px 13px; outline: none; width: 100%; transition: border-color 0.15s;
}
...
.btn-primary { background: var(--accent); color: #0d0f0e; font-weight: 600; padding: 10px 20px; font-family: 'Syne', sans-serif; }
.btn-primary:hover { background: #cdf58a; }
.btn-ghost { background: transparent; color: var(--text2); border: 1px solid var(--border2); padding: 8px 16px; }
...
.modal { background: var(--surface); border: 1px solid var(--border2); border-radius: var(--radius-xl); padding: 28px; width: 100%; max-width: 560px; max-height: 90vh; overflow-y: auto; }
.modal-lg { max-width: 720px; }
.sidebar { width: 220px; min-width: 220px; background: var(--surface); border-right: 1px solid var(--border); padding: 20px 12px; display: flex; flex-direction: column; gap: 4px; min-height: 100vh; }
...
.stat-card { background: var(--surface2); border: 1px solid var(--border); border-radius: var(--radius-lg); padding: 16px 20px; }
...
.exercise-row { background: var(--surface2); border: 1px solid var(--border); border-radius: var(--radius); padding: 14px 16px; margin-bottom: 10px; }
      ...
      <div style={{ width: "100%", maxWidth: "400px" }}>
        <div style={{ textAlign: "center", marginBottom: "36px" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "10px", marginBottom: "8px" }}>
            ...
            <span style={{ fontFamily: "Syne, sans-serif", fontSize: "22px", fontWeight: "800", color: "var(--text)", letterSpacing: "-0.02em" }}>CoachOS</span>
          </div>
          <p style={{ color: "var(--text2)", fontSize: "14px", marginTop: "6px" }}>Your personal coaching platform</p>
        </div>
        ...
        <div className="card" style={{ padding: "28px" }}>
          ...
          <button className="btn-primary w-full" style={{ padding: "11px 0", fontSize: "15px", width: "100%" }} onClick={handleLogin}>Sign in</button>
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      <div className="sidebar">
        <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 12px 20px" }}>
          ...
          <span style={{ fontFamily: "Syne, sans-serif", fontWeight: "800", fontSize: "16px", letterSpacing: "-0.02em" }}>CoachOS</span>
        </div>
        {navItems.map((n) => (
          <button key={n.id} className={"nav-item" + (isActive(n.id) ? " active" : "")}
            ...
            <span style={{ width: "20px", textAlign: "center" }}>{n.icon}</span> {n.label}
          </button>
        ))}
        <div style={{ flex: 1 }} />
        <div style={{ padding: "12px", borderTop: "1px solid var(--border)", marginTop: "8px" }}>
          <p style={{ fontSize: "12px", color: "var(--text3)" }}>Logged in as</p>
          <p style={{ fontSize: "14px", fontWeight: "500", color: "var(--text2)", marginTop: "2px" }}>{store.coach.name}</p>
        </div>
      </div>
...
function CoachDashboard({ store, openClient, openProgram }) {
  ...
  return (
    <div>
      <p style={{ color: "var(--text2)", marginBottom: "4px", fontSize: "14px" }}>Good to see you,</p>
      <h1 style={{ fontSize: "26px", fontWeight: "700", marginBottom: "28px" }}>{store.coach.name}</h1>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "10px", marginBottom: "28px" }}>
        {[
          { num: clients.length, label: "Active clients" },
          { num: programs.length, label: "Programs" },
          { num: totalExercises, label: "Exercises" },
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
        <div>
          <h2 style={{ fontSize: "18px", fontWeight: "700", marginBottom: "14px" }}>Recent clients</h2>
            ...
            return (
              <div key={c.id} className="client-row" onClick={() => openClient(c)}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: "12px", color: "var(--text3)", marginTop: "2px" }}>{progs.length} programs</p>
                </div>
        ...
        <div>
          <h2 style={{ fontSize: "18px", fontWeight: "700", marginBottom: "14px" }}>Recent programs</h2>
            ...
            return (
              <div key={p.id} className="client-row" onClick={() => openProgram(p)}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: "12px", color: "var(--text3)", marginTop: "2px" }}>{client ? client.name : "Unknown"} · {p.exercises.length} exercises</p>
                </div>
              </div>
            );
          })}
          {programs.length === 0 && <p style={{ color: "var(--text2)", fontSize: "14px" }}>No programs yet.</p>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "28px" }}>
        <h1 style={{ fontSize: "26px", fontWeight: "700" }}>Clients</h1>
        ...
        return (
          <div key={c.id} className="client-row" onClick={() => openClient(c)}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <p style={{ fontSize: "13px", color: "var(--text2)", marginTop: "4px" }}>{c.email}</p>
              {c.goal !== "" && <p style={{ fontSize: "11px", color: "var(--text3)", marginTop: "3px" }}>Goal: {c.goal}</p>}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <span className="chip">{progs.length} programs</span>
      ...
      {store.clients.length === 0 && (
        <div className="empty-state"><div className="empty-icon">👤</div><p>No clients yet. Add your first client!</p></div>
      )}
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "28px" }}>
        ...
        <div>
          <h1 style={{ fontSize: "26px", fontWeight: "700" }}>{client.name}</h1>
          <p style={{ fontSize: "13px", color: "var(--text2)" }}>{client.email} · Joined {client.joinDate}</p>
          {client.goal !== "" && <span className="tag tag-blue" style={{ marginTop: "6px", display: "inline-block" }}>{client.goal}</span>}
        </div>
        <div style={{ marginLeft: "auto", textAlign: "right" }}>
          <p style={{ fontSize: "11px", color: "var(--text3)" }}>Client password</p>
          <p style={{ fontSize: "13px", fontFamily: "monospace", background: "var(--surface2)", padding: "4px 10px", borderRadius: "6px", marginTop: "4px" }}>{client.password}</p>
        </div>
      </div>

      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px" }}>
        <h2 style={{ fontSize: "18px", fontWeight: "700" }}>Programs</h2>
      ...
      {programs.map((p) => (
        <div key={p.id} className="client-row" onClick={() => openProgram(p)}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: "13px", color: "var(--text2)", marginTop: "4px" }}>{p.exercises.length} exercises · {p.weeks} weeks</p>
          </div>
          <span className="tag tag-green">Edit</span>
        </div>
      ))}
      {programs.length === 0 && <div className="empty-state"><div className="empty-icon">📋</div><p>No programs yet.</p></div>}
...
function ProgramsList({ store, setStore, openProgram }) {
  ...
  return (
    <div>
      <h1 style={{ fontSize: "26px", fontWeight: "700", marginBottom: "28px" }}>All programs</h1>
      {store.programs.map((p) => {
        ...
        return (
          <div key={p.id} className="client-row" onClick={() => openProgram(p)}>
            ...
            <div style={{ flex: 1 }}>
              <p style={{ fontWeight: "500" }}>{p.name}</p>
              <p style={{ fontSize: "13px", color: "var(--text2)", marginTop: "4px" }}>{client ? client.name : "Unknown"} · {p.exercises.length} exercises · {p.weeks} weeks</p>
            </div>
...
function ProgramEditor({ program, store, setStore, setTab, setSelectedProgram }) {
  ...
  const deleteExercise = (id) => {
    save({ ...prog, exercises: prog.exercises.filter((e) => e.id !== id) });
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "6px" }}>
        <h1 style={{ fontSize: "26px", fontWeight: "700" }}>{prog.name}</h1>
        <span className="tag tag-blue">{prog.weeks}w</span>
      </div>
      <p style={{ fontSize: "13px", color: "var(--text2)", marginBottom: "28px" }}>Client: {client ? client.name : "Unknown"} · {prog.exercises.length} exercises</p>

      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
        <h2 style={{ fontSize: "18px", fontWeight: "700" }}>Exercises</h2>
      ...
      {prog.exercises.map((ex, idx) => (
        <div key={ex.id} className="exercise-row">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <span style={{ fontSize: "12px", color: "var(--text3)", fontFamily: "Syne, sans-serif", fontWeight: "700" }}>{"#" + (idx + 1)}</span>
              <input value={ex.name} onChange={(e) => updateExercise(ex.id, "name", e.target.value)}
                style={{ fontWeight: "600", fontSize: "15px", background: "transparent", border: "none", padding: "0", width: "auto", minWidth: "180px" }} />
              <span className={"tag " + (ex.type === "weight" ? "tag-green" : "tag-blue")}>{ex.type === "weight" ? "Weight" : "Time"}</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              {ex.videoUrl !== "" && (
                <button className="btn-ghost btn-sm" onClick={() => setShowVideoModal(ex)}>Preview video</button>
              )}
              <button className="btn-danger btn-sm" onClick={() => deleteExercise(ex.id)}>Remove</button>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 2fr", gap: "10px", marginBottom: "10px" }}>
            ...
            <div><label>Video URL (YouTube or Vimeo)</label><input value={ex.videoUrl} onChange={(e) => updateExercise(ex.id, "videoUrl", e.target.value)} placeholder="https://youtube.com/watch?v=..." /></div>
          </div>
      ...
      {prog.exercises.length === 0 && <div className="empty-state"><div className="empty-icon">🏋️</div><p>No exercises yet. Add your first movement!</p></div>}
      ...
      {showVideoModal !== null && (
        <div className="modal-overlay" onClick={() => setShowVideoModal(null)}>
          <div className="modal modal-lg" onClick={(e) => e.stopPropagation()}>
            ...
            {showVideoModal.notes !== "" && <p style={{ fontSize: "13px", color: "var(--text2)", marginTop: "12px" }}>Coach note: {showVideoModal.notes}</p>}
          </div>
        </div>
      )}
    </div>
...
function Settings({ store, setStore }) {
  ...
  return (
    <div style={{ maxWidth: "480px" }}>
      <h1 style={{ fontSize: "26px", fontWeight: "700", marginBottom: "28px" }}>Settings</h1>
      <div className="card">
      ...
      <div className="card" style={{ marginTop: "16px" }}>
        ...
        <p style={{ fontSize: "13px", color: "var(--text2)", marginBottom: "14px" }}>Delete all clients, programs and logs.</p>
...
function ClientApp({ store, setStore, clientId }) {
  ...
  if (prog) {
    prog.exercises.forEach((ex) => {
      ...
      if (exLog && exLog.sets) {
        exLog.sets.forEach((s) => { if (s.done) completedSets++; });
      }
    });
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <div style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <div style={{ width: "28px", height: "28px", background: "var(--accent)", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px" }}>⚡</div>
          <span style={{ fontFamily: "Syne, sans-serif", fontWeight: "800", fontSize: "16px" }}>CoachOS</span>
        </div>
              ...
              <div>
                <div style={{ marginBottom: "20px" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "6px" }}>
                    <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: "20px", fontWeight: "700" }}>{prog.name}</h2>
                ...
                <div style={{ marginBottom: "20px" }}>
                  <label style={{ marginBottom: "8px", display: "block" }}>Week</label>
                ...
                <div style={{ display: "flex", gap: "8px", marginBottom: "20px" }}>
                  {["workout", "history"].map((t) => (
                    <button key={t} onClick={() => setTab(t)}
                      style={{ padding: "7px 18px", borderRadius: "var(--radius)", fontFamily: "Syne, sans-serif", fontWeight: "600", fontSize: "13px",
                    ...
                    {prog.exercises.map((ex) => {
                      ...
                      return (
                        <div key={ex.id} className="exercise-row" style={{ marginBottom: "14px" }}>
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
                            <div>
                              <p style={{ fontWeight: "600", fontSize: "15px" }}>{ex.name}</p>
                              <p style={{ fontSize: "11px", color: "var(--text2)", marginTop: "2px" }}>
                                {ex.sets} sets · {ex.type === "weight" ? ex.reps + " reps" : ex.reps + "s"}
                              </p>
                              {ex.notes !== "" && <p style={{ fontSize: "11px", color: "var(--text3)", marginTop: "3px" }}>Coach: {ex.notes}</p>}
                            </div>
                            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                              <span style={{ fontSize: "12px", color: "var(--text2)" }}>{doneSets}/{ex.sets}</span>
                              {ex.videoUrl !== "" && (
                                <button className="btn-ghost btn-sm" onClick={() => setShowVideo(ex)}>Demo video</button>
                              )}
                            </div>
                          </div>

                          <div style={{ display: "grid", gridTemplateColumns: "32px 1fr 1fr 48px", gap: "6px", marginBottom: "4px" }}>
                            <div style={{ fontSize: "11px", color: "var(--text3)", paddingTop: "6px" }}>Set</div>
                            {ex.type === "weight" ? (
                              <div>
                                <div style={{ fontSize: "11px", color: "var(--text3)", paddingTop: "6px", textAlign: "center" }}>Reps</div>
                              </div>
                            ) : (
                              <div>
                                <div style={{ fontSize: "11px", color: "var(--text3)", paddingTop: "6px", textAlign: "center" }}>Seconds</div>
                              </div>
                            )}
                            {ex.type === "weight" ? (
                              <div style={{ fontSize: "11px", color: "var(--text3)", paddingTop: "6px", textAlign: "center" }}>Weight (lbs)</div>
                            ) : <div />}
                            <div style={{ fontSize: "11px", color: "var(--text3)", paddingTop: "6px", textAlign: "center" }}>Done</div>
                          </div>

                          {Array.from({ length: ex.sets }, (_, si) => {
                            const s = sets[si] || { reps: "", weight: "", time: "", done: false };
                            return (
                              <div key={si} style={{ display: "grid", gridTemplateColumns: "32px 1fr 1fr 48px", gap: "6px", marginBottom: "6px", alignItems: "center" }}>
                                ...
                                <div style={{ display: "flex", justifyContent: "center" }}>
                                  <button onClick={() => updateLog(prog.id, week, ex.id, si, "done", s.done ? false : true)}
                                    style={{ width: "32px", height: "32px", borderRadius: "50%",
                                      background: s.done ? "var(--accent)" : "var(--surface3)",
                                      border: "1px solid " + (s.done ? "var(--accent)" : "var(--border2)"),
                                      color: s.done ? "#0d0f0e" : "var(--text3)", fontSize: "16px",
  ...
  return (
    <div>
      ...
      {prog.exercises.map((ex) => {
        ...
        return (
          <div key={ex.id} className="exercise-row" style={{ marginBottom: "14px" }}>
            <p style={{ fontWeight: "600", marginBottom: "10px" }}>{ex.name}</p>
            {weekData.length === 0 ? (
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                  ...
                  <tbody>
                    {weekData.map((d, i) => {
                      ...
                      let improved = false;
                      if (prev !== null) {
                        if (ex.type === "weight" && d.maxWeight !== undefined && prev.maxWeight !== undefined) {
                          improved = d.maxWeight > prev.maxWeight;
                        } else if (ex.type === "time" && d.avgTime !== undefined && prev.avgTime !== undefined) {
                          improved = d.avgTime > prev.avgTime;
                        }
                      }
                      return (
                        <tr key={d.week} style={{ borderTop: "1px solid var(--border)" }}>
                          <td style={{ padding: "8px", fontWeight: "600", fontFamily: "Syne, sans-serif" }}>{"W" + d.week}</td>
                          <td style={{ padding: "8px", textAlign: "center", color: "var(--text2)" }}>{d.sets}</td>
                          {ex.type === "weight" ? (
                            <td style={{ padding: "8px", textAlign: "center", color: improved ? "var(--accent)" : "var(--text)" }}>
    ...
    <div>
      ...
      {session !== null && session.role === "coach" && (
        <div>
          ...
          <button onClick={() => setSession(null)} style={{ position: "fixed", bottom: "20px", right: "20px", background: "var(--surface2)", color: "var(--text2)", border: "1px solid var(--border2)", borderRadius: "var(--radius)", padding: "8px 14px", fontSize: "13px", cursor: "pointer", zIndex: 100 }}>Sign out</button>
        </div>
      )}
